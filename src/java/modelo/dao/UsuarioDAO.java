...
//            }
            ...
            while (rs.next()) {
                ...
                usuario.setNombre(rs.getString(2));
                usuario.setClave(rs.getString(3));
                usuario.setTb_perfil_usuario_id(rs.getString(4));
//                usuario.setTb_trabajador_id(rs.getInt(5));
//                usuario.setTb_cliente_id(rs.getInt(6));
            }
            rs.close();
    ...
    public boolean insertar(Usuario usuario) {
        try{
            return conexion.ejecutar(String.format("INSERT IGNORE INTO %s VALUES(?,?,?,?,?,?)", TABLA), new Object[]
                {null, usuario.getNombre(), usuario.getClave(), usuario.getPerfil_usuario(), usuario.getTrabajador(), usuario.getCliente()});
        }
        catch (Exception ex) {
            return false;
        }
    ...
    public boolean editar(Usuario usuario) {
        try {
            return conexion.ejecutar(String.format("UPDATE %s SET nombre=?, clave=?, perfil_usuario=?, ... , ... WHERE %s=?", TABLA, CLAVE_PRIMARIA), new Object[]{null, usuario.getNombre(), usuario.getClave(), usuario.getPerfil_usuario(), usuario.getTrabajador(), usuario.getCliente()});
        } catch (Exception ex) {
            return false;
        }
    }
